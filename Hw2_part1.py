...
#Функция для получения ваканции по id
async def get_vacancy_by_id(id, user_agent):
    ...
    asyncio.timeout(10)
    return soup
#Функция для запуска запросов для получения вакансий и их парсинга
async def main(ids):
    tasks = []
    data = []
    #Создаём задания для запуска для получения страниц вакансий по их id
    for id in ids:
...
try:
    ...
    #Получаем страницу со списком вакансий, по 20 штук на страницу и проходим 5 страниц
    for index in range (0, 5) :
        ...
        #Парсим из url id каждой вакансии
        ids = []
